...
const VacancyDao_1 = require("./VacancyDao");
...
const VacancyValidationSchema_1 = __importDefault(require("../../ValidationSchema/VacancyValidationSchema"));
...
const get = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const data = "";
        const dataReturn = yield (0, VacancyDao_1.getDao)(data);
...
const getId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const dataReturn = yield (0, VacancyDao_1.getIdDao)(id);
    let returnExist = yield (0, statusActive_services_1.statusActive)(dataReturn);
    if ((returnExist === null || returnExist === void 0 ? void 0 : returnExist.length) === 0)
...
const post = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    if (Array.isArray(req.body)) {
        try {
            for (let item of req.body) {
                const { error } = VacancyValidationSchema_1.default.validate(item);
                if (error) {
            ...
            const dataReturnS = yield (0, VacancyDao_1.postBulkDao)(values);
            if (!dataReturnS)
    ...
    if (!Array.isArray(req.body)) {
        let { error, value } = VacancyValidationSchema_1.default.validate(req.body);
        try {
            const currentTime = yield (0, today_services_1.today)();
            ...
            const dataReturnS = yield (0, VacancyDao_1.postDao)(value);
            if (!dataReturnS)
                return res.status(500).json(errorResponse);
            let returnExist = yield getAllAlways();
            if (!returnExist)
...
const update = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!createdAt)
            return res.status(500).json(errorResponse);
        let { error, value } = VacancyValidationSchema_1.default.validate(updateCreate);
        if (error)
            console.error(error.details);
        if (error)
            return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, VacancyDao_1.updateDao)(req.body, id);
        if (dataReturnS) {
...
const deletes = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const dataGetId = yield (0, VacancyDao_1.getIdDao)(id);
        if (!dataGetId)
            if (500)
                return res.status(500).json(errorResponse);
        ...
        if (!todos)
            if (500)
                return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, VacancyDao_1.deletesDao)(todos, id);
        if (dataReturnS) {
            const dataReturnGet = yield getAllAlways();
