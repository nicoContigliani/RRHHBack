    ...
    if (Array.isArray(req.body)) {
        try {
            for (let item of req.body) {
                const { error } = TypeTestValidationSchema_1.default.validate(item);
                if (error) {
            ...
            const dataReturnS = yield (0, TypeTestDao_1.postBulkDao)(values);
            if (!dataReturnS)
...
const update = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!createdAt)
            return res.status(500).json(errorResponse);
        let { error, value } = TypeTestValidationSchema_1.default.validate(updateCreate);
        if (error)
            console.error(error.details);
        if (error)
            return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, TypeTestDao_1.updateDao)(req.body, id);
        if (dataReturnS) {
...
const deletes = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const dataGetId = yield (0, TypeTestDao_1.getIdDao)(id);
        if (!dataGetId)
            if (500)
                return res.status(500).json(errorResponse);
        ...
        if (!todos)
            if (500)
                return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, TypeTestDao_1.deletesDao)(todos, id);
        if (dataReturnS) {
            const dataReturnGet = yield getAllAlways();
