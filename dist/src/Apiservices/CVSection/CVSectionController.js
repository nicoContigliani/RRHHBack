...
const CVSectionDao_1 = require("./CVSectionDao");
...
const CVSectionValidationSchema_1 = __importDefault(require("../../ValidationSchema/CVSectionValidationSchema"));
...
const get = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const data = "";
        const dataReturn = yield (0, CVSectionDao_1.getDao)(data);
...
const getId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const dataReturn = yield (0, CVSectionDao_1.getIdDao)(id);
    let returnExist = yield (0, statusActive_services_1.statusActive)(dataReturn);
    if ((returnExist === null || returnExist === void 0 ? void 0 : returnExist.length) === 0)
    ...
    try {
        ...
        if (isArray) {
            for (const obj of req.body) {
                const { error, value } = CVSectionValidationSchema_1.default.validate(obj);
                if (error) {
                ...
                const dataReturn = yield (0, CVSectionDao_1.postDao)(obj);
                if (!dataReturn)
                    return res.status(500).json(errorResponse);
            ...
            const { error, value } = CVSectionValidationSchema_1.default.validate(req.body);
            if (error) {
                console.error(error.details);
                return res.status(500).json(errorResponse);
            }
            ...
            const dataReturn = yield (0, CVSectionDao_1.postDao)(value);
            if (!dataReturn)
                return res.status(500).json(errorResponse);
            dataReturnS.push(dataReturn);
        }
...
const update = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!createdAt)
            return res.status(500).json(errorResponse);
        let { error, value } = CVSectionValidationSchema_1.default.validate(updateCreate);
        if (error)
            console.error(error.details);
        if (error)
            return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, CVSectionDao_1.updateDao)(req.body, id);
        if (dataReturnS) {
...
const deletes = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const dataGetId = yield (0, CVSectionDao_1.getIdDao)(id);
        if (!dataGetId)
            if (500)
                return res.status(500).json(errorResponse);
        ...
        if (!todos)
            if (500)
                return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, CVSectionDao_1.deletesDao)(todos, id);
        if (dataReturnS) {
            const dataReturnGet = yield getAllAlways();
