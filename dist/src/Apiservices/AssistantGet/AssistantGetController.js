...
const AssistantGetDao_1 = require("./AssistantGetDao");
...
const InterviewResponsibleValidationSchema_1 = __importDefault(require("../../ValidationSchema/InterviewResponsibleValidationSchema"));
...
const get = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const todo = yield (0, AssistantGetDto_1.formaterGet)(dataReturn);
        return res.status(200).json({ data: todo, message: (0, alert_services_1.AlertServices)("Success", "Find"), status: 200 });
    }
    catch (error) {
...
const getId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const dataReturn = yield (0, AssistantGetDao_1.getIdDao)(id);
    const todo = yield (0, AssistantGetDto_1.formaterGet)(dataReturn);
    // let returnExist = await statusActive(dataReturn)
...
const post = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    if (Array.isArray(req.body)) {
        try {
            for (let item of req.body) {
                const { error } = InterviewResponsibleValidationSchema_1.default.validate(item);
                if (error) {
            ...
            const dataReturnS = yield (0, AssistantGetDao_1.postBulkDao)(values);
            if (!dataReturnS)
    ...
    if (!Array.isArray(req.body)) {
        let { error, value } = InterviewResponsibleValidationSchema_1.default.validate(req.body);
        try {
            const currentTime = yield (0, today_services_1.today)();
            ...
            const dataReturnS = yield (0, AssistantGetDao_1.postDao)(value);
            if (!dataReturnS)
                return res.status(500).json(errorResponse);
            let returnExist = yield getAllAlways();
            if (!returnExist)
...
const update = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!createdAt)
            return res.status(500).json(errorResponse);
        let { error, value } = InterviewResponsibleValidationSchema_1.default.validate(updateCreate);
        if (error)
            console.error(error.details);
        if (error)
            return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, AssistantGetDao_1.updateDao)(req.body, id);
        if (dataReturnS) {
...
const deletes = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const dataGetId = yield (0, AssistantGetDao_1.getIdDao)(id);
        if (!dataGetId)
            if (500)
                return res.status(500).json(errorResponse);
        ...
        if (!todos)
            if (500)
                return res.status(500).json(errorResponse);
        const dataReturnS = yield (0, AssistantGetDao_1.deletesDao)(todos, id);
        if (dataReturnS) {
            const dataReturnGet = yield getAllAlways();
