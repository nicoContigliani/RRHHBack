...
import { getDao, getIdDao, postDao, updateDao, deletesDao } from './CompanyDao';
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    let { error, value } = CompanyValidationSchema.validate(req.body);
    try {
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        let { error, value } = CompanyValidationSchema.validate(updateCreate);
        if (error) return res.status(500).json(errorResponse);
