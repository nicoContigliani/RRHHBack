...
import { getDao, getIdDao, postDao, updateDao, deletesDao, postBulkDao } from './SectionTypeDao';
import { statusActive } from '../../services/statusActive.services';
import { AlertServices } from '../../services/alert.services';
import SectionTypeValidationSchema from '../../ValidationSchema/SectionTypeValidationSchema';
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (Array.isArray(req.body)) {
        try {
            for (let item of req.body) {
                const { error } = SectionTypeValidationSchema.validate(item);
                if (error) {
    ...
    if (!Array.isArray(req.body)) {
        let { error, value } = SectionTypeValidationSchema.validate(req.body);
        try {
            const currentTime = await today()
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        let { error, value } = SectionTypeValidationSchema.validate(updateCreate);
        if (error) console.error(error.details)
        if (error) return res.status(500).json(errorResponse);
