...
import { getDao, getIdDao, postDao, updateDao, deletesDao } from './CompanyVacancesDao';
import { statusActive } from '../../services/statusActive.services';
import { AlertServices } from '../../services/alert.services';
import CompanyVacancesValidationSchema from '../../ValidationSchema/CompanyVacancesValidationSchema';
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    let { error, value } = CompanyVacancesValidationSchema.validate(req.body);
    try {
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        let { error, value } = CompanyVacancesValidationSchema.validate(updateCreate);
        if (error) return res.status(500).json(errorResponse);
